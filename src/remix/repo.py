...
from utils      import uncompress_bz2_file, find_filename, rm, command
...
class Repo:
    ...
    def __get_repo_content( self ):
        ...
        if self.baseurl != "":
            ...
            baseurl = get_baseurl_from_mirror( self.mirrorlist )
    ...
    def __init__(self, repoline):
        self.name       = self.__get_value( repoline, "name"         )
        self.baseurl    = self.__get_value( repoline, "baseurl"      )
        self.mirrorlist = self.__get_value( repoline, "mirrorlist"   )
        if self.baseurl == "":
            if self.mirrorlist != "":
                self.baseurl = self.__get_baseurl_from_mirror( )
            else:
                 raise Exception( "[Error] repo need to provides a baseur or a mirrorlist" )
        self.type       = self.baseurl[ : self.baseurl.find(':') ]
...
class RepoData:
    ...
    def __get_primarydb( self ):
        url             = self.repo.baseurl + "/repodata/"
        primarydb_path  = ""
        if self.repo.type == "http":
            primarydb_url   = find_item_from_url( url, lambda x : x.name == 'a' and x["href"].endswith( "-primary.sqlite.bz2" ) )['href'].rstrip()
            compressedFile  = download_file( url + primarydb_url, self.directory )
            primarydb_path  = path.join( self.directory, "primary.sqlite")
    ...
    def __get_compsXML( self ):
        ...
        if self.repo.type == "http":
            ...
            if comps_url is not None:
                comps_file = download_file( url + comps_url['href'].rstrip(), self.directory )
        return comps_file
    ...
    def __init__(self, repo, repodata_dir, discinfo = ""):
        ...
        self.repo       = repo
        self.directory  = repodata_dir
        self.primarydb  = self.__get_primarydb()
        self.comps      = self.__get_compsXML()
        self.cursor     = connect(self.primarydb).cursor()
    ...
    def has_group( self, group_name ):
        ...
        if self.soup is not None:
            group_id    = self.soup.find( "id", text = group_name )
    ...
    def has_package_strict( self, name ):
        self.cursor.execute("SELECT count(name) FROM packages WHERE name = ?", (name,) )
        result  = False
    ...
    def has_package_like( self, name ):
        name    = name.replace( '*', '%' )
        self.cursor.execute("SELECT count(name) FROM packages WHERE name LIKE ?", (name,) )
        result  = False
        counter = self.cursor.fetchone()[0]
        if counter >= 1:
            result = True
    ...
    def get_package_url_strict( self, name ):
        self.cursor.execute("SELECT location_href FROM packages WHERE name = ?", (name,) )
        return self.cursor.fetchone()[0]


    def get_package_url_like( self, name ):
        name    = name.replace( '*', '%' )
        self.cursor.execute("SELECT location_href FROM packages WHERE name LIKE ?", (name,) )
    ...
    def get_package_version_strict( self, name ):
        self.cursor.execute("SELECT version FROM packages WHERE name = ?", (name,) )
    ...
    def get_package_version_like( self, name ):
        name    = name.replace( '*', '%' )
        self.cursor.execute("SELECT version FROM packages WHERE name LIKE ?", (name,) )
        return [ element[0] for element in self.cursor.fetchall() ]
    ...
    def get_package_key_strict(self, name ):
        key = ""
        self.cursor.execute("SELECT pkgKey FROM packages WHERE name = ?", (name,) )
    ...
    def get_packages_keys_like(self,  name):
        result = []
        name    = name.replace( '*', '%' )
        self.cursor.execute( "SELECT pkgKey FROM  packages WHERE name LIKE ?", (name,) )
